...
export default {
  ...
  methods: {
    ...
    submitForm() {
      ...
      let url = "retreats/category/store";
      if(this.id){
        url = `retreats/category/${this.id}`
      }
      this.busy = true;
      this.$Progress.start();
      axios.post(url, formData)
        ...
        .catch((error) => {
          let x = "Couldn't update category.";
          if(!this.id)
            x = "Couldn't create category."
          this.$toast.error(x, {
            position: "top-right",
          });
          this.$Progress.fail();
  ...
  created() {
    if(this.id>0){
      axios
        .get(`retreats/category/show/${this.id}`)
        .then(({ data }) => {
